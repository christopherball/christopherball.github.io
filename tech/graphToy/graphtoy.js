...
function Grapher() {
  ...
  function iMouseDown(e) {
    ...
    if (e.button == 0 && e.shiftKey == false) mMouseFunction = 1;
    else mMouseFunction = 2;
    mRefCx = mCx;
    mRefCy = mCy;
    mRefRa = mRa;
  ...
  function iCompile(id) {
    ...
    let inc5 = id >= 5 && strExpanded.includes("f5");
    if (inc5)
      strExpanded = strExpanded.replace(
        "f5",
        document.getElementById("formula5").value
      );
    let inc4 = id >= 4 && strExpanded.includes("f4");
    if (inc4)
      strExpanded = strExpanded.replace(
        "f4",
        document.getElementById("formula4").value
      );
    let inc3 = id >= 3 && strExpanded.includes("f3");
    if (inc3)
      strExpanded = strExpanded.replace(
        "f3",
        document.getElementById("formula3").value
      );
    let inc2 = id >= 2 && strExpanded.includes("f2");
    if (inc2)
      strExpanded = strExpanded.replace(
        "f2",
        document.getElementById("formula2").value
      );
    let inc1 = id >= 1 && strExpanded.includes("f1");
    if (inc1)
      strExpanded = strExpanded.replace(
        "f1",
  ...
  function iApplyGrid() {
    ...
    if (mShowAxes == 0) ele.textContent = "Grid Off";
    else if (mShowAxes == 1) ele.textContent = "Grid Dec";
    else if (mShowAxes == 2) ele.textContent = "Grid Bin";
  }
  ...
  function iDrawGraph(id, mycolor, mode) {
    ...
    if (mode === 1) mContext.lineWidth += 3.0;
  ...
  function iDraw() {
    ...
    let o = 0;
    for (let i = 0; i < 6; i++) {
      if (selectedID != i) order[o++] = i;
    }
    if (selectedID != -1) order[o] = selectedID;
